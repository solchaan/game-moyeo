...
export interface Participation { meetupId:number; participantCount:number; capacity:number; remainingCapacity:number; recruitmentStatus:'OPEN'|'CLOSED'; participationStatus:'OWNER'|'JOINED'|'NOT_JOINED'; closedReason:string|null; canParticipate:boolean; canCancel:boolean; canClose:boolean; availableActions:string[] }
